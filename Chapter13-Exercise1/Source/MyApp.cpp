...
MyApp::MyApp(HINSTANCE Instance) :
	...
	mVertexLayout(0)
{
	mWindowTitle = L"Chapter 13 Exercise 1";
}
...
void MyApp::DrawScene()
{
	...
	mImmediateContext->IASetVertexBuffers(0, 1, &mTriPatchVB, &stride, &offset);
...
void MyApp::BuildTrianglePatchBuffer()
{
	D3D11_BUFFER_DESC vbd;
	vbd.Usage = D3D11_USAGE_IMMUTABLE;
	vbd.ByteWidth = sizeof(DirectX::XMFLOAT3) * 6;
	vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vbd.CPUAccessFlags = 0;
	vbd.MiscFlags = 0;
	...
	HR(mDevice->CreateBuffer(&vbd, &vinitData, &mTriPatchVB));
}
