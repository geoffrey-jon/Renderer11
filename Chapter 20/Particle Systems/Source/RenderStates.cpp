...
void RenderStates::InitAll(ID3D11Device* device)
{
	...
	// Disable Depth Depth/Stencil State
	D3D11_DEPTH_STENCIL_DESC DisableDepthDSSDesc;
	DisableDepthDSSDesc.DepthEnable = false;
	DisableDepthDSSDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;
	DisableDepthDSSDesc.DepthFunc = D3D11_COMPARISON_LESS;
	DisableDepthDSSDesc.StencilEnable = false;

	HR(device->CreateDepthStencilState(&DisableDepthDSSDesc, &DisableDepthDSS));

	// No Depth Writes Depth/Stencil State
	D3D11_DEPTH_STENCIL_DESC NoDepthWritesDSSDesc;
	NoDepthWritesDSSDesc.DepthEnable = true;
	NoDepthWritesDSSDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;
	NoDepthWritesDSSDesc.DepthFunc = D3D11_COMPARISON_LESS;
	NoDepthWritesDSSDesc.StencilEnable = false;

	HR(device->CreateDepthStencilState(&NoDepthWritesDSSDesc, &NoDepthWritesDSS));
	...
	AdditiveBSDesc.RenderTarget[0].RenderTargetWriteMask = 0x0F;

	HR(device->CreateBlendState(&AdditiveBSDesc, &AdditiveBS));
